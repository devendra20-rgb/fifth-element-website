...
export default function CareerForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // Yahan aap apna SMTP/API logic daalenge
  ...
  if (submitted) {
    return (
      <div className="max-w-3xl mx-auto bg-white p-12 rounded-[3rem] shadow-2xl text-center border border-gray-100">
        ...
        <p className="text-gray-600 mb-8">Thank you for your interest. Our talent acquisition team will review your profile and get back to you shortly.</p>
        <button onClick={() => setSubmitted(false)} className="text-indigo-600 font-bold hover:underline">Apply for another role</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white p-8 md:p-16 rounded-[3rem] shadow-2xl border border-gray-100">
      <div className="mb-12">
        <h2 className="text-3xl font-black text-[#0f172a] mb-2 uppercase italic">Apply Now</h2>
        <p className="text-gray-500">Fill out the details below and attach your latest CV.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {/* Position Selection */}
        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-700 uppercase tracking-widest ml-1">Interested Position</label>
          <select name="position" className="w-full px-6 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all appearance-none cursor-pointer">
        ...
        {/* Resume Upload */}
        <div className="space-y-2">
          <label className="text-sm font-bold text-gray-700 uppercase tracking-widest ml-1">Resume / CV (PDF)</label>
